...
func TestDecodeLogRecordHead(t *testing.T) {
	...
	head, size := DecodeLogRecordHead(headerBuf)
	t.Log(head)
	t.Log(size)
	...
	// 测试 crc 的值
	assert.Equal(t, uint32(2732417296), head.crc)
	// 测试 LogRecord 类型
	assert.Equal(t, LogRecordNormal, head.recordType)
	// 测试 keySize
	assert.Equal(t, uint32(4), head.keySize)
	// 测试 valueSize
	assert.Equal(t, uint32(6), head.valueSize)
